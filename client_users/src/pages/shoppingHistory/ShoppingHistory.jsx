...
const HistorialDeCompra = () => {
  ...
  useEffect(() => {
    (async () => {
      const response = await axios.get(`ecommerce-c2nccxhew-andrecarosu-hotmailcom.vercel.app/email?email=${email}`);
      const idUsuario = response.data[0].user_id;
      console.log(3, idUsuario);
      setTimeout(async () => {
        const response2 = await axios.get(`ecommerce-c2nccxhew-andrecarosu-hotmailcom.vercel.app/venta/${idUsuario}`);
